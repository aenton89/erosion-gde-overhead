...
var erosion: PipeErosionOptimized
...
func erode_terrain() -> void:
	...
	P.profile("PipeOptimized", "erode", Callable(erosion, "erode"), [heightmap, erosion_iterations, erosion_settings])
	
	mesh_instance.mesh = P.profile("PipeOptimized", "generate_mesh", Callable(renderer, "generate_mesh"), [heightmap])
	apply_material()
